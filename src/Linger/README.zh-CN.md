...
- **JSON 配置工厂**: `JsonOptions` 提供统一的 JSON 序列化配置
  - 详细文档: `Linger/Json/JsonOptions.README.zh-CN.md`
...
提供的 `LeftJoin`、`RightJoin`、`FullJoin` 等方法与 .NET 10 标准完全兼容，升级时零代码修改。通过条件编译 `#if !NET10_0_OR_GREATER` 自动切换实现。
...
| **集合扩展** | `LeftJoin`、`RightJoin`、`FullJoin` (.NET 10 前瞻) | `Extensions/Collection/IEnumerableExtensions.Polyfills.cs` |
