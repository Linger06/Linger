...
public class PathHelperBaseTests
{
    ...
    public void RemoveConsecutiveSeparators_ShouldRemoveDuplicateSeparators()
    {
        ...
        // 确认结果符合预期
        // RemoveConsecutiveSeparators: 只移除连续斜杠，无连续斜杠的输入直接返回原值（快速路径）
        Assert.Equal($"path{separator}to{separator}file", result1);  // 有连续斜杠，被处理
        Assert.Equal("path/to/file", result2);                        // 无连续斜杠，快速路径直接返回
